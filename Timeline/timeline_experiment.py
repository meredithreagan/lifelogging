...
with Loop(imgstim) as trial:
    with Parallel():
        Image(source=trial.current['stim'], duration=trial.current['dur'], top=exp.screen.top+5)

    with UntilDone():
        with Serial():
            with Parallel():
                MouseCursor()
                s1 = Slider(min=-100, max=100, value=0, width=slider_width, top=400)
            ...
            with UntilDone():
                ...
                with Parallel():
                    ...
                    MouseCursor()
                    s2 = Slider(min=0, max=7, value=3.5, width=slider_width, top=300)
                ...
                with UntilDone():
                    ...
                    with Parallel():
                        ...
                        MouseCursor()
                        s3 = Slider(min=0, max=24, value=12, width=slider_width, top=200)
                    ...
                    with UntilDone():
                        ...
                        with Parallel():
                            ...
                            MouseCursor()
                            s4 = Slider(min=0, max=100, value=50, width=slider_width, top=100)
                            ...
                            lblvconf = Label(text="Not at all confident", center_x=exp.screen.left+100, top=40)
                            lblnotconf = Label(text="Very confident", center_x=exp.screen.right-100, top=40)
...
"""Log(name="timeline",
            trial_num=trial_current['trial_num']"""
